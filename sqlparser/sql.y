...
%token <empty> CALL
%token <empty> ALL DISTINCT AS EXISTS IN IS LIKE BETWEEN NULL ASC DESC VALUES INTO DUPLICATE KEY DEFAULT SET LOCK COLLATE
...
%type <statement> insert_statement update_statement delete_statement set_statement call_statement
...
%type <valExpr> value value_expression set_value
%type <tuple> tuple
...
| SET comment_opt NAMES set_value
  {
    $$ = &Set{Comments: Comments($2), Exprs: UpdateExprs{&UpdateExpr{Name: &ColName{Name:[]byte("names")}, Expr: $4}}}
  }
| SET comment_opt NAMES set_value COLLATE set_value
  {
    $$ = &Set{Comments: Comments($2), Exprs: UpdateExprs{&UpdateExpr{Name: &ColName{Name:[]byte("names")}, Expr: $4}, &UpdateExpr{Name: &ColName{Name:[]byte("collate")}, Expr: $6}}}
  }
...
call_statement:
  CALL dml_table_expression '(' value_expression_list ')'
  {
	$$ = &Call{Procedure: $2, Values: $4}
  }
| CALL dml_table_expression '(' ')'
  {
    $$ = &Call{Procedure: $2}
  }
