...
func (plan *RoutingPlan) shardListFromPlan() (shardList []int) {
	...
	if plan.rule.Type == router.NoneRuleType || len(plan.rule.Nodes) == 1 {
...
func checkUpdateExprs(exprs UpdateExprs, rule *router.Rule) {
	if rule.Type == router.NoneRuleType {
		return
...
func getRoutingPlan(statement Statement, router *router.Router) (plan *RoutingPlan) {
	...
	case *Delete:
		...
		where = stmt.Where
	case *Call:
		plan.rule = router.GetRule(stmt.GetTableName())
	}
	...
	//TODO: 对于不带where条件的语句，怎么看待他们的分发规则
	plan.fullList = makeList(0, len(plan.rule.Nodes))
...
func (plan *RoutingPlan) routingAnalyzeBoolean(node BoolExpr) []int {
	...
	case *AndExpr:
		...
		log.Info(String(node.Left))
		log.Info(String(node.Right))
...
func (plan *RoutingPlan) routingAnalyzeValue(valExpr ValExpr) int {
	...
	case ValTuple:
		...
		return LIST_NODE
	case StrVal, NumVal, *NullVal, ValArg:
		return VALUE_NODE
	}
	return OTHER_NODE
}
