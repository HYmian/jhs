...
func (c *Conn) handleQuery(sql string) (err error) {
	...
	case *sqlparser.Select:
		return c.jhs_handleSelect(v, sql, nil)
		return c.handleSelect(v, sql, nil)
	case *sqlparser.Insert:
		return c.jhs_handleExec(v, sql, nil)
...
func (c *Conn) getShardList(stmt sqlparser.Statement, bindVars map[string]interface{}) ([]*Node, error) {
	...
	ns, err := sqlparser.GetStmtShardList(stmt, router.NewRouter(c.server.rules, c.server.default_), bindVars)
	if err != nil {
		return nil, err
	}
...
func (c *Conn) limitSelectResult(r *Resultset, stmt *sqlparser.Select) error {
	...
	if offset+count > int64(len(r.Values)) && count < int64(len(r.Values)) {
